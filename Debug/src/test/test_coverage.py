# 覆盖率采集测试脚本，自动启动 QEMU 并执行多轮 SSH/SMTP 测试采集覆盖率数据
...
def run_coverage():
    ...
    ssh_current_hashes = set()
    smtp_current_hashes = set()
    ratio_rows = []
    coverage_round = 0
    ...
    try:
        ...
        coverage_round = collect_rounds(coverage_sock, "ssh", "random", coverage_random_rounds, coverage_round, ssh_current_hashes, ssh_initial_hashes, smtp_initial_hashes, ratio_rows)
        coverage_round = collect_rounds(coverage_sock, "smtp", "random", coverage_random_rounds, coverage_round, smtp_current_hashes, ssh_initial_hashes, smtp_initial_hashes, ratio_rows)
